...
async function sleepButTheSame(ms: any) {
...
*/
async function mySumFunction(sum1: any, sum2: any): Promise<number> {
    await sleep(2000);
    if (sum1 + sum2 < 0) {
...
async function main() {
    ...
    try {
      ...
      */
      let result = await mySumFunction(1, 2);
    ...
    try {
       ...
       * On attend le résultat avec "await"
       */
      let result = await mySumFunction(1, -10);
     ...
     */
    try {
      let result1 = await mySumFunction(0, 10);
      console.log(`Le résultat de l'addition : `, result1);
      result1 = await mySumFunction(result1, 10);
      console.log(`Le résultat de l'addition : `, result1);
      result1 = await mySumFunction(result1, 10);
      console.log(`Le résultat de l'addition : `, result1);
      result1 = await mySumFunction(result1, 10);
